...
void dfs(int node, int par){
    ...
    for(int child:g[node]){
        ...
        // it means that child is not connected to any ancesstor of node 
            // ie. if the if condition is false :: even if we remove this node then it is connected 
            // with a ancesstor of the currnode: and hence removing this node will not affected  no
            // of connected component so no bridfe if condition fails
